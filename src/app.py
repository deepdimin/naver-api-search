...
if not NAVER_CLIENT_ID or not NAVER_CLIENT_SECRET:  # API 키 존재 여부 확인
    ...
    # 디버깅 도움말 표시 (TOML 에러 방지를 위해 매우 안전하게 접근)
    with st.expander("🛠️ 시스템 진단 정보 (디버깅용)"):
        try:
            secrets_keys = list(st.secrets.keys())
            st.write(f"- **인식된 Secrets 키 목록**: `{secrets_keys if secrets_keys else '비어있음'}`")
        except Exception as e:
            st.warning(f"⚠️ **Secrets 설정 파일에 문법 오류가 있습니다.**")
            st.error(f"에러 메시지: {e}")
            st.info("Streamlit Cloud 설정창에 입력한 내용에 따옴표(`\"`)나 등호(`=`)가 빠졌는지 확인해 주세요.")
    ...
    st.info("""
    **💡 해결 방법:**
    1. Streamlit Cloud의 **Settings > Secrets**에 아래 형식을 맞춰 입력했는지 확인해 주세요.
    ```toml
    NAVER_CLIENT_ID = "내_아이디"
    NAVER_CLIENT_SECRET = "내_시크릿"
    ```
    2. GitHub에 혹시 `.streamlit` 폴더를 직접 올리셨다면 삭제해 주세요.
    """)
    st.stop()  # 앱 실행 중단
